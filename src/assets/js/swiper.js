...
function initializeSwiperCarousels() {
  ...
  swiperContainers.forEach((swiperContainer) => {
    ...
    const centerSlides =
      swiperContainer.getAttribute("data-center-slides") === "true";
    const effect = swiperContainer.getAttribute("data-effect") || "slide";
    const thumbsEnabled =
      swiperContainer.getAttribute("data-thumbs") === "true";
    ...
    if (breakpointsData) {
      try {
        ...
        // Error parsing breakpoints - using defaults
      }
    }
    ...
    if (thumbsEnabled) {
      ...
      if (
        thumbsContainer &&
        thumbsContainer.classList.contains("swiper-thumbs")
      ) {
